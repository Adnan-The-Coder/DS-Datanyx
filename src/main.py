...
# Make probability predictions (for prioritization)
y_prob = pipeline.predict_proba(X_test)[:, 1]  # Probabilities for the 'Immediate Care Required' class

# Add predictions to the test set
df_test = X_test.copy()
df_test['Immediate Care Required Probability'] = y_prob
df_test['Actual'] = y_test

# Prioritize animals needing first attention (sort by the probability of requiring immediate care)
df_test['Priority'] = df_test['Immediate Care Required Probability'].rank(ascending=False, method='dense')

# Add 'Animal Type' to the output for better readability
df_test['Animal Type'] = X_test['Animal Type'].values

# Sort by Priority (highest priority first)
df_test_sorted = df_test.sort_values(by='Priority', ascending=True)

# Display the prioritized animals
print("Prioritized List of Animals Needing Immediate Care:")
print(df_test_sorted[['Animal Type', 'Immediate Care Required Probability', 'Priority', 'Actual']])
